...
const ARTIST_ID = "2pwxA6FXPCRje8le8719pQ";
...
async function fetchAlbumTracks(token, albums, limitAlbums = 24) {
  const tracks = [];
  const slice = albums.slice(0, limitAlbums);
...
async function writeEmptyFallback(reason) {
  const catalog = {
    ...
    tracks: [],
    latestRelease: null,
    counts: { total: 0, albums: 0, singles: 0, tracks: 0 },
    updatedAt: null,
    note: reason,
  };
  await mkdir(path.dirname(OUT), { recursive: true });
  await writeFile(OUT, `${JSON.stringify(catalog, null, 2)}\n`, "utf8");
  console.warn(`[sync-spotify] Wrote empty ALY catalog (${reason})`);
}
...
async function main() {
  ...
  if (!clientId || !clientSecret) {
    try {
      ...
      if (existing.targetArtistId === ARTIST_ID && Number(existing.schemaVersion) >= 4) {
        console.warn("[sync-spotify] Missing credentials — keeping valid empty/prior ALY catalog");
        return;
      }
    } catch {
      /* fall through */
    }
    await writeEmptyFallback("missing-credentials");
    return;
  }
  ...
  try {
    ...
    const tracks = await fetchAlbumTracks(token, releases, 24);
    await writeCatalog(releases, tracks);
  } catch (error) {
    console.error("[sync-spotify] Failed:", error.message);
    await writeEmptyFallback(`sync-failed:${error.message}`);
    process.exitCode = 0;
  }
}
