...
const EXPECTED_ARTIST_NAME = "ALY";
const SPOTIFY_MARKET = "TR";
...
async function fetchAllPages<TItem>(
  ...
  while (nextPath) {
    const requestPath = nextPath.startsWith("http")
      ? nextPath.replace(API_BASE, "")
      : nextPath;
    const page = await spotifyFetch<{ items: TItem[]; next: string | null }>(
      requestPath,
      token,
    );
...
async function buildSpotifyPayload(
  ...
  const releaseBuilders: Array<{
    release: Omit<NormalizedRelease, "trackIds">;
    trackIds: string[];
    albumGroup: string;
