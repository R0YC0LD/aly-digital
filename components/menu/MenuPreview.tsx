...
function resolvePreview(activeId: string, latestCover: string | null): PreviewState {
  ...
  let src = assetPath(siteConfig.media.hero);
  let label = item.label;
  let mode: "image" | "fallback" = "image";

  if (item.preview === "journey") {
    src = assetPath(siteConfig.media.journey[0] || siteConfig.media.hero);
  }
  if (item.preview === "instagram") src = assetPath(siteConfig.media.instagram);
  if (item.preview === "hero") src = assetPath(siteConfig.media.hero);
