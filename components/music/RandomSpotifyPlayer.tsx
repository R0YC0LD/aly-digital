...
const SCRIPT_FLAG = "__alySpotifyIframeApiLoading";
...
declare global {
  interface Window {
    ...
    __alySpotifyIframeApi?: IFrameAPI;
    [SCRIPT_FLAG]?: boolean;
  }
}
...
export function RandomSpotifyPlayer({ tracks }: Props) {
  ...
  useEffect(() => {
    ...
    const api = window.__alySpotifyIframeApi;
    if (api) {
      setupController(api);
    } else {
      const previous = window.onSpotifyIframeApiReady;
      window.onSpotifyIframeApiReady = (IFrameAPI) => {
        window.__alySpotifyIframeApi = IFrameAPI;
        previous?.(IFrameAPI);
      ...
      } else if (window.__alySpotifyIframeApi) {
        setupController(window.__alySpotifyIframeApi);
      }
    }
      ...
      <div className={styles.wrap}>
        <div className={styles.head}>
          <div>
            ...
            <p className={styles.sub}>ALY single listesi şu anda yenileniyor.</p>
          </div>
